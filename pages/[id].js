...
const Dhaaira = (props) => {
    ...
    const fetchapi = async () => {
        let response = await fetch(`${process.env.DOMAINSS}/cluster-map/apiv1/${router.query.id}.json`)
        response = await response.json()
        console.log(response, "fsafa")
    ...
    return (
      ...
      <div className="container pb-3 mt-3">
        ...
        {Object.keys(state.data.clusters).map((c)=>{
          <div><h1>{state.data.clusters[c]}</h1></div>
        })}
...
export async function getStaticProps({ params }) {
  ...
  console.log(dhaairas[params.id])
    ...
    { dhaaira,
      pageHeader: {
        ...
        metas: [
           ...
           { property: 'og:title', content: `${dhaairas[params.id]} | ޖުޑީޝަލް ދާއިރާތަކުގެ ޗާޓު` },
           {
              property: 'og:image',
